		...
		var req = http.request(options, function(res) {
			...
			res.on('end', function () {
				if (res.statusCode == 412) {
					if (TRACE && DETAIL) {
						console.log("Wemo reports temporarily unavailable, retrying in 5s");
						setTimeout(makeRequest.call(this,options,callback),5000);
					}
				}
	...
	this.service._resubscribe(this.sid, function(err, buf) {
		if (err) {
			...
			operation.attempt(function(currentAttempt) {
				self.service.subscribe(function(err, data) {
		      if (operation.retry(err)) {
			      console.log("FAILURE: lost subscription could not be reestablished yet");
						return;

		      }
		      console.log("SUCCESS: lost subscription reestablished");
